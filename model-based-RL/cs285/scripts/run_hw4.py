...
def collect_mbpo_rollout(
    ...
    for _ in range(rollout_len):
        # get action
        ac = sac_agent.get_action(np.array(ob))[0]
        acs.append(ac)
        obs.append(ob)
        
        # Ensure that obs is a 2D array
        obs_list = [mb_agent.get_dynamics_predictions(i, np.array(ob).reshape(1, -1), np.array(acs)) for i in range(mb_agent.ensemble_size)]
        next_ob = np.mean(obs_list, axis=0)
        print("next ob shape", next_ob.shape)
        # get reward
    ...
    for itr in range(config["num_iters"]):
        ...
        for _ in tqdm.trange(
            ...
            for i in range(mb_agent.ensemble_size):
                ...
                #print(f'Dynamics Model {i+1} - Step Loss: {step_loss}')
                #mb_agent.update(i, batch["observations"], batch["actions"], batch["next_observations"])
        
            all_losses.append(np.mean(step_losses))
            #print("last loss", all_losses[-1])

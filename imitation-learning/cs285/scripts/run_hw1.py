...
def run_training_loop(params):
    ...
    for itr in range(params['n_iter']):
        ...
        if itr == 0:
            ...
            # TODOs: collect `params['batch_size']` transitions
            # HINT: use utils.sample_trajectories
            # get data by running agent in environment
            # use agent's policy (actor) to collect data
            # TODOs: implement missing parts of utils.sample_trajectory
            bs = params['batch_size']
            max_len = params['ep_len']
            paths, envsteps_this_batch = utils.sample_trajectories(env, actor, bs, max_len)

            # relabel the collected obs with actions from a provided expert policy
            if params['do_dagger']:
                print("\nRelabelling collected observations with labels from an expert policy...")

                # TODOs: relabel collected observations (from our policy) with labels from expert policy
                # HINT: query the policy (using the get_action function) with paths[i]["observation"]
                # and replace paths[i]["action"] with these expert labels
                # For each collected path, query the expert policy to get expert labels
        ...
        print("paths ", [path["reward"] for path in replay_buffer.paths])
        ...
        for _ in range(params['num_agent_train_steps_per_iter']):
          ...
          length = len(replay_buffer.acs)
          size = params['train_batch_size']
          batch_indices = np.random.permutation(length)[:size]

          # get {o} and {a} corresponding to sampled indices
          observations = []
          actions = []
          for idx in batch_indices:
              obs = replay_buffer.obs[idx]
              action = replay_buffer.acs[idx]
              observations.append(obs)
              actions.append(action)
              
          # convert observations and actions to NumPy arrays
          print("obs list og: ", len(observations))
          print("ac list og: ", len(actions))

          # use the sampled data to train an agent
        #   print("ob_batch shape: ", ob_batch.shape, ", ac_batch shape: ", ac_batch.shape)
          train_log = actor.update(np.array(observations), np.array(actions))
...
def main():
    ...
    ##################################
    print("CREATE DIRECTORY FOR LOGGING\n")
    if args.do_dagger:

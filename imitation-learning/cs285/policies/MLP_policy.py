...
class MLPPolicySL(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def forward(self, observation: torch.FloatTensor) -> Any:
        ...
        # pass obs through the mean_net to predict the action mean
        """
        observation = observation.view(-1, self.ob_dim)  # Reshape the observation if needed
        action_mean = self.mean_net(observation)
        
        # create a normal dist with predicted mean and exp(logstd) for std dev
        action_std = torch.exp(self.logstd)
        action_distribution = torch.distributions.Normal(action_mean, action_std)
        
        # sample an action from dist
        ...
        #return action
        u = self.mean_net(observation)
        sigma = torch.exp(self.logstd)
        return distributions.Normal(u, sigma)
    ...
    def update(self, observations, actions):
        ...
        print(f"shape of observations {observations.size()} and shape of actions {actions.size()}")
        
        predicted_actions = self.forward(observations)

        loss = -predicted_actions.log_prob(actions).mean()
        loss.backward()
        self.optimizer.step()
